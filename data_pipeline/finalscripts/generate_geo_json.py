...
def process_dataframe(df):
	...
	dfh = merge_shapes_with_dataframe(df)
	dfh = get_tweets_per_hour(dfh)
	...
	stop.extend(['...',',,',',,,','..', 't','c','i','I','a', '@','.'])
	dfr['tokens'] = dfr['tokens'].apply(lambda x: remove_stopwords(x))
...
def remove_stopwords(token_row):
    '''
    INPUT: a string or list of tokens, such as from a row from the 'tokens' column
    OUTPUT: a list of tokens with the stopwords removed'''
    
    if type(token_row) != list:
        token_row = [token_row]
    return [i for i in token_row if i not in stop]
